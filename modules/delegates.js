...
private.attachApi = function () {
	...
	router.post('/forging/enable', function (req, res) {
		...
		library.scheme.validate(body, {
			...
			var keypair = ed.MakeKeypair(crypto.createHash('sha3').update(body.secret, 'utf8').digest());
...
private.loadMyDelegates = function (cb) {
	...
	async.eachSeries(secrets, function (secret, cb) {
		var keypair = ed.MakeKeypair(crypto.createHash('sha3').update(secret, 'utf8').digest());
...
Delegates.prototype.generateDelegateList = function (height, cb) {
	private.getKeysSortByVote(function (err, truncDelegateList) {
		...
		var currentSeed = crypto.createHash('sha3').update(seedSource, 'utf8').digest();
		for (var i = 0, delCount = truncDelegateList.length; i < delCount; i++) {
			...
			currentSeed = crypto.createHash('sha3').update(currentSeed).digest();
		}
...
shared.addDelegate = function (req, cb) {
	...
	library.scheme.validate(body, {
		...
		var hash = crypto.createHash('sha3').update(body.secret, 'utf8').digest();
				...
				modules.accounts.getAccount({publicKey: body.multisigAccountPublicKey}, function (err, account) {
					...
					modules.accounts.getAccount({publicKey: keypair.publicKey}, function (err, requester) {
						...
						if (requester.secondSignature) {
							var secondHash = crypto.createHash('sha3').update(body.secondSecret, 'utf8').digest();
